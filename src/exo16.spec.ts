...
describe('Let the tests on arrays begin !', () => {
  ...
  test('checks if the array only contains String element', () => {
    mockArray = [
      'Lord of the rings : the return of the king',
      'Into the wild', 'Alien the 8th passenger',
      'Tombeau des lucioles',
      'fight club',
    ];
    mockArray.forEach((word) => {
  ...
  test(`checks if the elements in mockArray respect that the strings
   are the same with the right order as solutionArray`, () => {
    mockArray = ['Blue', 'Red', 'Blue', 'Yellow'];
    const solutionArray = ['Blue', 'Red', 'Blue', 'Yellow'];
    expect(mockArray.filter((x, index) => !solutionArray[index].includes(x))).toEqual([]);
    expect(mockArray.join(',')).toEqual(solutionArray.join(','));
  });
});
...
describe('Let the tests on objects begin !', () => {
  ...
  test(`Create a test that ensures that your 'command' object contains at least \`fries: true\` and \`sauce:
   "Hannibal"\` without checking properties one by one`, () => {
    mockObject = {
      sauce: 'Hannibal',
      fries: true,
      kebab: 'Double',
